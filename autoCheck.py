...
def startAtuoCheckIn(config):
    ...
    if loginInfo==None:
        ...
        for clazz in clazzInfo:
            if config['checkClazzIds']==None or len(config['checkClazzIds'])==0 or clazz['id'] in config['checkClazzIds']:
                clazzList.append({'clazz_id':clazz['id'],'clazz_name':clazz['course_name'], 'flag':1})
        ...
        while(True):
            ...
            for clazz in clazzList:
                ...
                if msg!=None and msg['result_msg']=="OK" and clazz['flag']==1:
